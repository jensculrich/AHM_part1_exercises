# code pasted 
# from https://github.com/mikemeredith/AHM_code/blob/main/AHM1_ch01/AHM1_01.1.R
...
my_function <- function(N, n, p){
  
  count <- rbinom(size = N, n = n, p = p)
  
  return(max(count))
}

count <- matrix(ncol = 3, nrow = 100)
...
for(i in 1:nrow(count)){ 
  ...
  p <- 0.5
  # p <- rbeta(1, 2, 2)
  max_count <- my_function(N, n, p)

  count[i,] <- cbind(N, p, max_count)
}

plot(count[,1], count[,3])
plot(count[,2], count[,3])
